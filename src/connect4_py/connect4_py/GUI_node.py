...
class Connect4ROSNode(Node):
    ...
    # ── Existing publish helpers (unchanged) ───────────────────────────────
...
class Connect4GUI(QWidget):
    ...
    def apply_styles(self):
        self.setStyleSheet("""
            ...
            QLabel { color: white; font-size: 14px; padding: 3px; }
            QLabel#titleLabel {
                font-size: 24px; font-weight: bold; color: #FFD600;
                padding: 12px; background-color: #002B66; border-radius: 10px;
            }
            QLabel#sectionTitle {
                font-size: 18px; font-weight: bold; color: #FFD600; padding-top: 10px;
            }
            QFrame#statusPanel {
                background-color: #002B66; border: 3px solid #FFD600;
                border-radius: 14px; padding: 12px;
            }
            QPushButton {
                font-weight: bold; border-radius: 8px; padding: 8px; min-height: 28px;
            }
            QPushButton#yellowButton {
                background-color: #FFD600; color: #111111; border: 2px solid #C9A900;
            }
            QPushButton#yellowButton:hover { background-color: #FFE75C; }
            QPushButton#redButton {
                background-color: #E53935; color: white; border: 2px solid #B71C1C;
            }
            QPushButton#redButton:hover { background-color: #FF5252; }
            QPushButton:disabled {
                background-color: #777777; color: #CCCCCC; border: 2px solid #555555;
            }
            QComboBox {
                background-color: white; color: #111111;
                padding: 6px; border-radius: 6px; min-height: 24px;
            }
            QTextEdit {
                background-color: #F7F7F7; color: #111111;
                border: 2px solid #FFD600; border-radius: 8px;
    ...
    def get_opponent_name(self):
        if self.system_mode == "IRL":   return "Robot"
        elif self.system_mode == "XR":  return "XR Player"
        return "Opponent"

    def update_score_label(self):
        self.score_label.setText(
            f"Score: Human {self.human_score} vs {self.get_opponent_name()} {self.opponent_score}")
    ...
    def show_hint(self):
        if not self.game_active:
            self.hint_label.setText("Hint: Game is not active"); return
        if self.selected_mode != "Easy":
            self.hint_label.setText("Hint: Disabled in Hard mode"); return
        if self.current_turn != "Player":
            self.hint_label.setText("Hint: Only available on the human turn"); return
        hint_result = self.game.get_best_hint_move(self.game.PLAYER_1)
        if hint_result is None:
            self.hint_label.setText("Hint: No valid moves available"); return
        best_col, reason = hint_result
        display_col = best_col + 1
        if reason == "win":
            self.hint_label.setText(f"Hint: Play column {display_col} to win")
        elif reason == "block":
            self.hint_label.setText(f"Hint: Play column {display_col} to block the opponent")
        else:
            self.hint_label.setText(f"Hint: Play column {display_col} to build toward 4 in a row")

    def set_difficulty(self, difficulty):
        if self.game_active:
            self.log.append("Cannot change difficulty while game is active")
            self.difficulty_box.setCurrentText(self.selected_mode); return
    ...
    def set_system_mode(self, mode):
        if self.game_active:
            ...
            self.system_mode_box.setCurrentText(self.system_mode); return
    ...
    def handle_human_move(self, column):
        if not self.game_active:
            self.log.append(f"Ignored human move in column {column + 1}: game is not active"); return
        if self.current_turn != "Player":
            self.log.append(f"Ignored human move in column {column + 1}: not player's turn"); return
        if column < 0 or column >= self.game.COLS:
            self.log.append(f"Ignored human move: column {column + 1} is outside 1-7"); return
        move_success = self.game.drop_piece(column, self.game.PLAYER_1)
        if not move_success:
            self.log.append(f"Ignored human move: column {column + 1} is full or invalid"); return
    ...
    def handle_robot_move(self, column):
        if self.system_mode == "XR":
            self.log.append("Ignored autonomous robot move because XR mode is active"); return
        if not self.game_active:
            self.log.append(f"Ignored robot move in column {column + 1}: game is not active"); return
        if self.current_turn != "Robot":
            self.log.append(f"Ignored robot move in column {column + 1}: not robot's turn"); return
        if column < 0 or column >= self.game.COLS:
            self.log.append(f"Ignored robot move: column {column + 1} is outside 1-7"); return
        move_success = self.game.drop_piece(column, self.game.PLAYER_2)
        if not move_success:
            self.log.append(f"Ignored robot move: column {column + 1} is full or invalid"); return
    ...
    def handle_xr_move(self, column):
        if self.system_mode != "XR":
            self.log.append("Ignored XR player move because IRL mode is active"); return
        if not self.game_active:
            self.log.append(f"Ignored XR move in column {column + 1}: game is not active"); return
        if self.current_turn != "Robot":
            self.log.append(f"Ignored XR move in column {column + 1}: not XR player's turn"); return
        if column < 0 or column >= self.game.COLS:
            self.log.append(f"Ignored XR move: column {column + 1} is outside 1-7"); return
        move_success = self.game.drop_piece(column, self.game.PLAYER_2)
        if not move_success:
            self.log.append(f"Ignored XR move: column {column + 1} is full or invalid"); return
    ...
    def end_game(self):
        ...
        if self.game.winner == self.game.PLAYER_1:
            ...
            self.log.append("GAME OVER: Robot wins!" if self.system_mode == "IRL" else "GAME OVER: XR player wins!")
        else:
    ...
    def handle_robot_status(self, status):
        if not self.game_active:
            self.log.append(f"Ignored robot status '{status}': game is not active"); return
